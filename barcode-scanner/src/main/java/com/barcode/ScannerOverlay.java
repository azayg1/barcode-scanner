...
public class ScannerOverlay extends ViewGroup {
    ...
    public ScannerOverlay(Context context, AttributeSet attrs, int defStyle) {
        ...
        int lineColor = a.getColor(R.styleable.ScannerOverlay_line_color, ContextCompat.getColor(context, R.color.scanner_line));
        int pointColor = a.getColor(R.styleable.ScannerOverlay_point_color, ContextCompat.getColor(context, R.color.scanner_line));
        int lineWidth = a.getInteger(R.styleable.ScannerOverlay_line_width, getResources().getInteger(R.integer.line_width));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < 10; i++) {
            canvas.drawPoint(left + random.nextInt(dpToPx(rectWidth)), top + random.nextInt(dpToPx(rectWidth)), point);
        }
